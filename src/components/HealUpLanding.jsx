    ...
    const fetchImages = async () => {
      try {
        ...
        const imageUrls = res.data
          ...
          .map(meal => `https://healupbackend-production.up.railway.app/uploads/${meal.photo}`);
        setImages(imageUrls);
        ...
        <div className="related-grid">
          {related.map((item, idx) => (
            <div key={idx} className="related-card" onClick={() => navigate(`/meal/${item.id}`)}>
              <img src={`https://healupbackend-production.up.railway.app/uploads/${item.photo}`} alt={item.name} />
