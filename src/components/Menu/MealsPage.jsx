...
export default function MealsPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingMealId) {
        await axios.put(`http://healupbackend-production.up.railway.app/api/meals/${editingMealId}`, data, {
          headers: { "Content-Type": "multipart/form-data" }
        });
        setEditingMealId(null);
      } else {
        await axios.post("http://healupbackend-production.up.railway.app/api/meals", data, {
          headers: { "Content-Type": "multipart/form-data" }
        });
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://healupbackend-production.up.railway.app/api/meals/${id}`);
      fetchMeals();
    } catch (err) {
            ...
            {meals.map((meal) => (
              <div key={meal.id} style={styles.card}>
                <div style={styles.imageContainer}>
                  <img
                    src={meal.photo ? `http://healupbackend-production.up.railway.app/uploads/${meal.photo}` : "/placeholder.png"}
